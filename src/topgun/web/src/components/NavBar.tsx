...
export default function NavBar() {
  ...
  return (
    <nav className="relative z-20 flex items-center justify-between px-6 py-4 border-b border-border-dim bg-base/80 backdrop-blur-sm">
      <Link to="/dashboard" className="font-mono text-sm font-bold tracking-[0.35em] text-amber-tac hover:text-amber-dim transition-colors">
        TOPGUN
      </Link>

      <div className="flex items-center gap-2 text-xs">
        {location.pathname !== "/dashboard" && (
          <Link to="/dashboard" className="font-mono text-text-secondary hover:text-amber-tac transition-colors tracking-widest uppercase mr-4">
            Dashboard
          </Link>
        )}
        {user && (
